-- Count every unique drink recorded during the current month while retaining
-- the separate count of drinks first added to the user's history this month.
...
BEGIN
  ...
  RETURN (
    ...
    month_lights AS (
      SELECT ld.*, ma.activity_at
      FROM month_activity ma
      JOIN light_details ld ON ld.light_id = ma.light_id
    ),
    month_new_lights AS (
      SELECT *
      FROM month_lights
    ...
    first_new_style AS (
      SELECT fs.style, fs.first_recorded_at, mnl.light_id
      FROM first_style_dates fs
      JOIN LATERAL (
        SELECT light_id
        FROM month_new_lights
    ...
    month_new_bar_count AS (
      SELECT count(DISTINCT uv.tenant_id)::integer AS count
      FROM public.user_drink_venues uv
      JOIN month_new_lights mnl ON mnl.light_id = uv.light_id
      WHERE uv.user_id = v_user_id
        AND uv.first_drank_at >= v_month_start
        AND uv.first_drank_at < v_month_end
    ),
    month_activity_bar_count AS (
      SELECT count(DISTINCT uv.tenant_id)::integer AS count
      FROM public.user_drink_venues uv
      WHERE uv.user_id = v_user_id
        AND uv.first_drank_at >= v_month_start
    ...
    SELECT jsonb_build_object(
      ...
      'month', jsonb_build_object(
        ...
        'drink_count', (SELECT count(*) FROM month_lights),
        'new_drink_count', (SELECT count(*) FROM month_new_lights),
        'bar_count', (SELECT count FROM month_new_bar_count),
        'activity_bar_count', (SELECT count FROM month_activity_bar_count),
        'style_counts', coalesce((
          SELECT jsonb_agg(jsonb_build_object('style', style, 'count', count) ORDER BY count DESC, style ASC)
          FROM month_styles
        ), '[]'::jsonb),
        'first_drink_id', (SELECT light_id FROM month_lights ORDER BY activity_at ASC, light_id ASC LIMIT 1),
        ...
        'top_style_drink_id', (
          ...
          ORDER BY ml.activity_at ASC, ml.light_id ASC
          LIMIT 1
        ),
        'latest_drink_id', (SELECT light_id FROM month_lights ORDER BY activity_at DESC, light_id DESC LIMIT 1),
        'drinks', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'light_id', mnl.light_id,
            'name', mnl.name,
            'brewery', mnl.brewery,
            'beer_style', mnl.beer_style,
            'image_url', mnl.image_url,
            'recorded_at', mnl.first_lit_at
          ) ORDER BY mnl.first_lit_at DESC, mnl.light_id DESC)
          FROM month_new_lights mnl
        ), '[]'::jsonb),
        'activity_drinks', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'recorded_at', ml.activity_at
          ) ORDER BY ml.activity_at DESC, ml.light_id DESC)
          FROM month_lights ml
        ), '[]'::jsonb)
      )
    )
