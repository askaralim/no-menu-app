-- Soft-archive drink_serving_options when order_items still reference them.
-- Hard DELETE remains when no order_items point at the row.

ALTER TABLE public.drink_serving_options
  ADD COLUMN IF NOT EXISTS archived_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_drink_serving_options_active
...
-- Keep existing payload shape; only hide archived servings from the editor.
