-- Collab breweries: expose only on bar tonight taplist (get_public_taplist_drinks).
-- Do NOT return collab_breweries on search / NEW ON TAP payloads so submitted
-- consumer builds that already call formatBreweryWithCollab stay primary-only
-- on those compact surfaces. Additive on drinks; removing the field from
-- search/new-taps is display-only (no schema break).
  ...
  RETURN (
    WITH base AS (
      SELECT
        ...
        coalesce(p.collab_breweries, '{}'::text[]) AS collab_breweries,
        ...
        jsonb_build_object(
          ...
          'beer', CASE
            WHEN b.brewery IS NULL
              AND coalesce(cardinality(b.collab_breweries), 0) = 0
            ...
            ELSE jsonb_build_object(
              'brewery', b.brewery,
              'collab_breweries', to_jsonb(b.collab_breweries),
...
-- NEW ON TAP: primary brewery only (no collab_breweries in payload).
...
GRANT EXECUTE ON FUNCTION public.get_public_taplist_new_drinks(text) TO anon, authenticated;

-- Search results: primary brewery only. Collab names may still match the query
-- (no UI field), so findability stays without changing the brewery line.
        ...
        FROM (
          ...
          WHERE t.status = 'active'
            ...
            AND (
              ...
              OR exists (
                SELECT 1
                FROM unnest(coalesce(p.collab_breweries, '{}'::text[])) AS collab_name
                WHERE collab_name ILIKE v_pattern
              )
