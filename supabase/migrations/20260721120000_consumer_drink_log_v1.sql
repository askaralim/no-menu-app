...
UPDATE public.taplist_public_cities
SET country_code = 'CN'
WHERE country_code IS NULL AND lower(trim(country)) IN ('china', '中国');

CREATE UNIQUE INDEX IF NOT EXISTS taplist_public_cities_id_uidx
  ON public.taplist_public_cities(id);

CREATE INDEX IF NOT EXISTS taplist_public_cities_country_code_idx
  ON public.taplist_public_cities(country_code);
...
REVOKE ALL ON FUNCTION public.reconcile_provisional_drink(uuid, uuid) FROM PUBLIC;
