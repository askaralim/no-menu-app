...
function resolveDrinkName(drink: DrinkRow, product: ProductRow | null | undefined) {
  return drink.display_name?.trim() || product?.name?.trim() || drink.name
}

function resolveBreweryName(
  drink: DrinkRow,
  product: ProductRow | null | undefined,
  profile: ProfileRow | null | undefined,
) {
  const brewery =
    product?.brewery?.trim() ||
    product?.brand_name?.trim() ||
    profile?.brewery?.trim() ||
    drink.brand_name?.trim() ||
    null
  return brewery || null
}

function formatSingleNewTapBody(drinkName: string, brewery: string | null, style: string | null) {
  const suffix = '，点按查看这款新酒。'
  if (brewery && style) return `${brewery} · ${drinkName} · ${style}${suffix}`
  if (brewery) return `${brewery} · ${drinkName}${suffix}`
  if (style) return `${drinkName} · ${style}${suffix}`
  return `${drinkName}已加入今晚酒单，点按查看。`
}
...
async function createBatchDeliveries(admin: ReturnType<typeof createClient>, batchId: string, tenantId: string) {
  ...
  const { data: drinkRows, error: drinkError } = await admin
    .from('drinks')
    .select('id, name, display_name, brand_name, product_id, category_id, enabled, is_public_visible, public_status')
    .in('id', drinkIds)
  ...
  const [{ data: profileRows, error: profileError }, { data: productRows, error: productError }] = await Promise.all([
    admin.from('drink_beer_profiles').select('drink_id, brewery, beer_style').in('drink_id', validDrinkIds),
    productIds.length > 0
      ? admin.from('drink_products').select('id, name, brewery, brand_name, beer_style').in('id', productIds).eq('status', 'active')
  ...
  const drinkNames = validDrinks.map((drink) => {
    ...
    return resolveDrinkName(drink, product)
  })
  const singleProduct = singleDrink?.product_id ? productsById.get(singleDrink.product_id) : null
  const singleProfile = singleDrink ? profilesByDrinkId.get(singleDrink.id) : null
  const singleStyle = singleDrink
    ? singleProduct?.beer_style?.trim() || singleProfile?.beer_style?.trim() || null
    : null
  const singleBrewery = singleDrink
    ? resolveBreweryName(singleDrink, singleProduct, singleProfile)
    : null
  const title = singleDrink
    ? `${barName} 上新`
    : `${barName} 今晚上新 ${validDrinks.length} 款`
  const body = singleDrink
    ? formatSingleNewTapBody(drinkNames[0], singleBrewery, singleStyle)
