...
--
-- Beer:
--   双尾酿造/Twin Tails | 失重-鲜酒花 | DDH双倍西海岸IPA | ABV 8.6 | 苏州
--
-- Notes:
...
menu_rows AS (
  ...
  FROM (
    VALUES
      (1, '双尾酿造/Twin Tails'::text, '失重-鲜酒花'::text, 'DDH双倍西海岸IPA'::text, 8.6::numeric, NULL::integer, '苏州'::text)
  ) AS v(row_no, brewery, beer_name, beer_style, abv, ibu, country)
),
rows_to_insert AS (
  SELECT
    m.*,
...
-- Verify:
-- SELECT d.sort_order, d.brand_name, d.name, p.beer_style, p.abv, p.ibu, p.country, d.price
-- FROM public.drinks d
-- LEFT JOIN public.drink_beer_profiles p ON p.drink_id = d.id
-- WHERE d.tenant_id = '831db2a1-ee47-4d88-9c0b-3e19a5668d6d'
--   AND d.brand_name = '双尾酿造/Twin Tails'
--   AND d.name = '失重-鲜酒花';
